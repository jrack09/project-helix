...
export default async function DrugCoveragePage() {
  ...
  const [warnings, missedDose, storage, doseEsc, seThresholds, seWindows, sites, oral, sources] = await Promise.all([
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="text-2xl font-bold mt-2">PIP field coverage</h1>
        <p className="text-sm text-muted-foreground mt-1">
          Per-drug presence of the structured fields exposed via <code>/api/public/drugs/[slug]</code>.
          Numbers are row counts; checks indicate a scalar field is set. Click a drug to fill gaps in its edit tabs.
        </p>
                    ...
                    <TableRow key={drug.id} className="group">
                      ...
                      <TableCell className="sticky right-0 z-10 bg-card group-hover:bg-muted/50 shadow-[-1px_0_0_0_hsl(var(--border))] text-right">
                        {missing === 0
                          ...
                          : <Badge variant={missing > 5 ? 'destructive' : 'warning'}>{missing}</Badge>}
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
